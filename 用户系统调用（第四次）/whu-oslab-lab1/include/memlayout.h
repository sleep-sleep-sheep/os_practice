...
// 1. 中断优先级寄存器（每个中断1个，32位）
#define PLIC_PRIORITY(id)    (PLIC_BASE + (id) * 4)  // 1~7有效，0禁止

// 2. 中断挂起寄存器（每个32位对应32个中断，按需扩展）
#define PLIC_PENDING         (PLIC_BASE + 0x1000)

// 3. 中断使能寄存器（S模式：64位/核心，步长0x8）- 修正步长
#define PLIC_SENABLE(hart)   (PLIC_BASE + 0x2080 + (hart) * 8)  // 原0x100→0x8

// 4. 优先级阈值寄存器（S模式：32位/核心，步长0x4）
#define PLIC_SPRIORITY(hart) (PLIC_BASE + 0x201000 + (hart) * 4)  // 原0x2000→0x4（单核心也兼容）

// 5. 中断认领/完成寄存器（S模式：32位/核心，完成比认领高4字节）- 补充SCOMPLETE
#define PLIC_SCLAIM(hart)    (PLIC_BASE + 0x201004 + (hart) * 4)  // 认领：读
#define PLIC_SCOMPLETE(hart) (PLIC_BASE + 0x201008 + (hart) * 4)  // 完成：写（比认领高4字节）

// 外设中断ID（根据硬件确认，QEMU virt平台UART是10）
#define UART_IRQ 10
